...
print(f"\nПолная таблица сохранена в файл: 'diabetes_predictions.csv'")
...
print("Графики сохранены в файл: 'diabetes_regression_plots.png'")
...
print(f"   - Выбор основан на максимальной корреляции с целевой переменной")

print(f"\n3. РЕЗУЛЬТАТЫ РЕГРЕССИИ:")
print(f"   - Уравнение: target = {sklearn_intercept:.2f} + {sklearn_coef:.2f} * {selected_feature}")
print(f"   - Обе реализации дают идентичные результаты")
print(f"   - R² = {r2_sklearn:.4f} (модель объясняет {r2_sklearn*100:.1f}% дисперсии)")

print(f"\n4. СОХРАНЕННЫЕ ФАЙЛЫ:")
print(f"   - diabetes_predictions.csv - полная таблица предсказаний")
print(f"   - diabetes_regression_plots.png - все графики")

print(f"\n5. ИНТЕРПРЕТАЦИЯ:")
print(f"   - Коэффициент {sklearn_coef:.2f} показывает, что при увеличении")
print(f"     '{selected_feature}' на 1 стандартное отклонение, target")
print(f"     увеличивается на {sklearn_coef:.2f} единиц")
