...
def main():
    ...
    print("\n" + "="*60)
    print("ДЕТАЛЬНЫЙ АНАЛИЗ ОШИБОК (Scikit-Learn)")
    print("="*60)
    ...
    print(f"   - Обе реализации дают практически одинаковые результаты")
    print(f"   - Разница в R²: {abs(r2_sk - r2_man):.10f}")
    print(f"   - Разница в MAE: {abs(mae_sk - mae_man):.10f}")
    print(f"\n3. РЕКОМЕНДАЦИИ:")
    print(f"   - Модель имеет удовлетворительное качество (R² = {r2_sk:.3f})")
    print(f"   - Для улучшения можно рассмотреть:")
    print(f"     а) Добавление других признаков")
    print(f"     б) Использование полиномиальной регрессии")
    print(f"     в) Попробовать другие алгоритмы (деревья, ансамбли)")
    print(f"\n4. ИНТЕРПРЕТАЦИЯ КОЭФФИЦИЕНТОВ:")
    print(f"   - Уравнение: y = {sklearn_model.intercept_:.2f} + "
          f"{sklearn_model.coef_[0]:.2f} * {selected_feature}")
    print(f"   - При увеличении {selected_feature} на 1, target изменяется "
          f"на {sklearn_model.coef_[0]:.2f}")
    plt.show()
